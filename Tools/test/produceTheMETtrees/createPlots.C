...
void createPlots(){
  outputFile = new TFile("allplotsNewMiniAod.root", "RECREATE");
  chain = new TChain("probeMuons");
  //chain->Add("/tmp/hbrun/muonTreeFile_*.root");
  chain->Add("probeMuonTree.root");
  chainBad = new TChain("badMuons");
  chainBad->Add("probeMuonTree.root");

  TString typeRECO = "promptRECO";

  TString tag = typeRECO+"allMuons";
  ...
  tag = typeRECO+"_goodMuonsClosest";
